    ...
    class Projectile(pygame.sprite.Sprite):
        def __init__(self, group, x, y, rotation, velocity, color):
            ...
            if self.friendly:
                ...
                self.vely *= 2.5
                pl.cooldown = 0
                pl.xvel -= velocity[0] / 10
                pl.yvel -= velocity[1] / 10
            ...
            if self.friendly:
                ...
                for enemy in melee_enemies:
                    if pygame.sprite.collide_mask(self, enemy) and enemy.active and not enemy.fading:
                        ...
                        self.explode()
                        break
                for enemy in enemy_projectiles:
                    if enemy != self.friendly:
    ...
    try:
        while running:
            ...
            for event in pygame.event.get():
                ...
                if event.type == pygame.MOUSEMOTION:
                    ...
                    if event.key == pygame.K_m:
                        ...
                        space = True
                    elif event.key == pygame.K_p or event.type == pygame.ACTIVEEVENT  and event.gain == 0:
                        while True:
                            for e in pygame.event.get():
                                if e.type == pygame.QUIT:
                                    ...
                                    if e.key == pygame.K_a:
                                        ...
                                        space = True
                                    elif e.key == pygame.K_p or e.type == pygame.ACTIVEEVENT and e.gain == 0:
                                        break
            ...
            if mouse_pos is not None:
                if flag:
                    ratio = 4 / sqrt(
                        (mouse_pos[1] - pl.rect.y - 19) ** 2 + (mouse_pos[0] - pl.rect.x - 17) ** 2)
                    Projectile(projectiles, pl.rect.x + 17, pl.rect.y + 19,
                               -degrees(
                                   atan2(mouse_pos[1] - pl.rect.y - 19, mouse_pos[0] - pl.rect.x - 17)),
                               (ratio * (mouse_pos[0] - pl.rect.x - 17),
                                ratio * (mouse_pos[1] - pl.rect.y - 19)),
                               pygame.Color('red'))
                    ratio = 4 / sqrt(
                        (mouse_pos[1] - pl.rect.y - 19) ** 2 + (mouse_pos[0] - pl.rect.x - 17) ** 2)
                    Projectile(projectiles, pl.rect.x + 17, pl.rect.y + 19,
                               -degrees(
                                   atan2(mouse_pos[1] - pl.rect.y - 19,
                                         mouse_pos[0] - pl.rect.x - 17)) * 1.1,
                               (ratio * (mouse_pos[0] - pl.rect.x - 17),
                                ratio * (mouse_pos[1] - pl.rect.y - 19) * 1.1),
                               pygame.Color('red'))
                    ratio = 4 / sqrt(
                        (mouse_pos[1] - pl.rect.y - 19) ** 2 + (mouse_pos[0] - pl.rect.x - 17) ** 2)
                    Projectile(projectiles, pl.rect.x + 17, pl.rect.y + 19,
                               -degrees(
                                   atan2(mouse_pos[1] - pl.rect.y - 19,
                                         mouse_pos[0] - pl.rect.x - 17)) * 1.2,
                               (ratio * (mouse_pos[0] - pl.rect.x - 17),
                                ratio * (mouse_pos[1] - pl.rect.y - 19) * 1.2),
                               pygame.Color('red'))
